...
# --- CONFIGURACIÓN v81.0 (ELO SYSTEM & HYBRID POWER) ---
...
class OmniHybridBot:
    def __init__(self):
        ...
        print("--- ENGINE v81.0 ELO POWER STARTED ---", flush=True)
        self.send_msg(f"🔧 <b>INICIANDO v81.0 (ELO + SoT)</b>\n📂 Ruta CSV: {HISTORY_FILE}\nEstado SDK: {SDK_STATUS}")
    ...
    def _init_history_file(self):
        if not os.path.exists(HISTORY_FILE):
            ...
            except Exception as e: print(f"Error CSV init: {e}")
    ...
    # --- SISTEMA ELO (NUEVO MOTOR) ---
    def calculate_elo_ratings(self, df):
        """Calcula el Rating ELO histórico para toda la temporada"""
        elo_ratings = {} # Dict {Team: ELO}
        base_elo = 1500
        k_factor = 20 # Peso de cada partido
        
        # Inicializar equipos
        teams = pd.concat([df['HomeTeam'], df['AwayTeam']]).unique()
        for t in teams: elo_ratings[t] = base_elo
        
        # Ordenar por fecha para calcular cronológicamente
        if 'Date' in df.columns:
            # Asumimos formato dd/mm/yy o dd/mm/yyyy segun football-data
            try: df = df.sort_values(by='Date')
            except: pass # Si falla el sort, procesamos como venga
            
        for _, row in df.iterrows():
            h_team = row['HomeTeam']
            a_team = row['AwayTeam']
            
            # Si hay goles, procesamos
            if pd.notna(row['FTHG']) and pd.notna(row['FTAG']):
                h_elo = elo_ratings.get(h_team, base_elo)
                a_elo = elo_ratings.get(a_team, base_elo)
                
                # Resultado real (1=Win, 0.5=Draw, 0=Loss)
                if row['FTHG'] > row['FTAG']: h_res = 1; a_res = 0
                elif row['FTHG'] == row['FTAG']: h_res = 0.5; a_res = 0.5
                else: h_res = 0; a_res = 1
                
                # Resultado Esperado (Fórmula ELO)
    ...
    # --- CÁLCULO HÍBRIDO (GOLES + Tiros a Puerta) ---
    def calculate_team_stats(self, df, team):
        ...
        for i, (_, row) in enumerate(matches.iterrows()):
            ...
            # Si no hay dato de tiros (ligas menores), usamos Goles * 3 como proxy
            
            if row['HomeTeam'] == team:
                goals = row['FTHG']
                shots = row.get('HST', goals * 3) # Fallback si no hay dato
                if pd.isna(shots): shots = goals * 3 # Doble check
                
                # FÓRMULA DE ATAQUE: 60% Goles + 40% (Tiros/3)
                # Dividimos tiros entre 3 porque aprox 3 tiros a puerta = 1 gol
                att_power = (goals * 0.60) + ((shots / 3) * 0.40)
                
                # Para defensa, hacemos lo mismo con lo que recibió
                goals_conc = row['FTAG']
                shots_conc = row.get('AST', goals_conc * 3)
                if pd.isna(shots_conc): shots_conc = goals_conc * 3
                def_weakness = (goals_conc * 0.60) + ((shots_conc / 3) * 0.40)
                
            else: # Away logic
                goals = row['FTAG']
                shots = row.get('AST', goals * 3)
                if pd.isna(shots): shots = goals * 3
                att_power = (goals * 0.60) + ((shots / 3) * 0.40)
                
                goals_conc = row['FTHG']
                shots_conc = row.get('HST', goals_conc * 3)
                if pd.isna(shots_conc): shots_conc = goals_conc * 3
                def_weakness = (goals_conc * 0.60) + ((shots_conc / 3) * 0.40)

            w_att += att_power * weight
            w_def += def_weakness * weight
    ...
    def get_league_data(self, div):
        ...
        try:
            ...
            # Promedio de goles global (para normalizar)
            if len(matches_played) > 0: 
                avg_g = matches_played.FTHG.mean() + matches_played.FTAG.mean()
            else: avg_g = 2.5
            
            # 1. Calcular ELO Ratings
            elo_map = self.calculate_elo_ratings(matches_played)
            
            # 2. Calcular Stats Híbridas
            teams = pd.concat([df['HomeTeam'], df['AwayTeam']]).unique()
            team_stats = {}
            avg_att_league = 0; avg_def_league = 0; cnt = 0
            
            for t in teams:
                a, d = self.calculate_team_stats(matches_played, t)
                team_stats[t] = {'att': a, 'def': d}
                avg_att_league += a; avg_def_league += d; cnt += 1
            
            if cnt > 0: avg_att_league /= cnt; avg_def_league /= cnt
            else: avg_att_league = 1; avg_def_league = 1
            
            # Normalizar
            norm_stats = {t: {'att': s['att']/avg_att_league, 'def': s['def']/avg_def_league} for t, s in team_stats.items()}
            
            return {'stats': norm_stats, 'elo': elo_map, 'teams': teams, 'raw_df': df, 'avg_g': avg_g}
        except: return None

    # --- SIMULACIÓN CON ELO BOOST ---
    def simulate_match(self, home, away, league_data, market_odds):
        ...
        # Factor ELO: Si hay mucha diferencia, ajustamos la fuerza
        elo_diff = h_elo - a_elo
        elo_factor_h = 1.0
        elo_factor_a = 1.0
        
        # Si Home es mucho mejor (+100 puntos ELO)
        if elo_diff > 100:
            elo_factor_h = 1.10 # +10% ataque
            elo_factor_a = 0.90 # -10% ataque rival
        # Si Away es mucho mejor
        elif elo_diff < -100:
            elo_factor_h = 0.90
            elo_factor_a = 1.10
        ...
        # Fórmula Maestra: Stats Híbridas * Factor ELO * Factor Localía (1.10)
        ...
        return {
            ...
            'gcs': gcs, 'cs': most_common, 'lambdas': (lambda_h, lambda_a), 
            'elo': (h_elo, a_elo) # Guardamos ELO para mostrar
        }
    ...
    def find_best_value(self, sim, odds):
        ...
        if sim['ah'][1] > 0.90: handicap_candidates.append({'pick': "Handicap A +1.5", 'odd': 1.15, 'ev': 0.05})

        best_handi = handicap_candidates[0]['pick'] if handicap_candidates else None
    ...
    def run_analysis(self):
        ...
        self.send_msg(f"🔎 <b>Analizando {len(daily)} partidos (v81.0 ELO)...</b>")
