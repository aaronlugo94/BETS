...
# --- CONFIGURACIÓN v80.0 (FINAL POLISH & PERSISTENCE) ---
...
# --- 💾 PERSISTENCIA RAILWAY (LO ÚNICO NUEVO) ---
VOLUME_PATH = "/app/data" 
if os.path.exists(VOLUME_PATH):
    HISTORY_FILE = os.path.join(VOLUME_PATH, "historial_omni_v80.csv")
    print(f"💾 USANDO VOLUMEN PERSISTENTE: {HISTORY_FILE}", flush=True)
else:
    HISTORY_FILE = "historial_omni_v80.csv"
    print("⚠️ USANDO ALMACENAMIENTO EFÍMERO (Crea un Volume en Railway para arreglar esto)", flush=True)
...
class OmniHybridBot:
    ...
    def _init_history_file(self):
        # Usamos try-except por si Railway tiene permisos raros
        try:
            ...
            print(f"❌ Error fatal creando CSV: {e}", flush=True)
    ...
    def run_audit(self):
        ...
        # Leemos CSV y actualizamos resultados
        try:
            with open(HISTORY_FILE, 'r', encoding='utf-8') as f:
                reader = csv.reader(f); header = next(reader); rows.append(header)
                for row in reader:
                    status = row[9]
                    # Si está pendiente (VALID/0) o incluso BACKUP, intentamos resolver
                        ...
                        if data and not data['raw_df'].empty:
                            ...
                            if not match.empty:
                                ...
                                if res in ["WIN", "LOSS", "PUSH"]:
                                    ...
                                    # Solo auditamos si fue un pick "VALID" (donde pusimos dinero)
                                    if stake > 0:
    ...
    def run_analysis(self):
        ...
        print(f"🚀 Iniciando v80.0 GHOST + CLEANER + PERSISTENCE: {today}", flush=True)
